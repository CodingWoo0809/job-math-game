...
const ui={hud:document.querySelector("#map25-hud"),objective:document.querySelector("#map25-objective"),prompt:document.querySelector("#map25-prompt"),promptText:document.querySelector("#map25-prompt-text"),toast:document.querySelector("#map25-toast"),start:document.querySelector("#map25-start"),startButton:document.querySelector("#map25-start-button"),dialogue:document.querySelector("#map25-dialogue"),avatar:document.querySelector("#map25-avatar"),speaker:document.querySelector("#map25-speaker"),dialogueText:document.querySelector("#map25-dialogue-text"),dialogueNext:document.querySelector("#map25-dialogue-next"),panel:document.querySelector("#age-panel"),close:document.querySelector("#age-close"),solveStage:document.querySelector("#solve-stage"),success:document.querySelector("#age-success"),solveStep:document.querySelector("#solve-step"),checkStep:document.querySelector("#check-step"),answerForm:document.querySelector("#answer-form"),answerFeedback:document.querySelector("#answer-feedback"),answerHint:document.querySelector("#answer-hint"),dropChain:document.querySelector("#drop-chain"),complete:document.querySelector("#map25-complete"),replay:document.querySelector("#map25-replay"),next:document.querySelector("#map25-next"),touch:document.querySelector("#map25-touch"),left:document.querySelector("#map25-left"),right:document.querySelector("#map25-right"),interact:document.querySelector("#map25-interact")};
...
function resetGame(){state=makeState();ui.answerForm.reset();ui.answerFeedback.textContent="";ui.solveStage.classList.remove("is-hidden");ui.success.classList.add("is-hidden");ui.solveStep.className="is-active";ui.checkStep.className="";ui.complete.classList.add("is-hidden");ui.start.classList.remove("is-hidden");ui.hud.classList.add("is-hidden");ui.touch.classList.add("is-hidden");ui.objective.textContent="골목을 막고 있는 형제에게 다가가자."}
function keyDown(event){if(event.target.matches?.("input,button"))return;if(["ArrowLeft","ArrowRight","a","A","d","D","e","E","Enter"," "].includes(event.key))event.preventDefault();if(event.key==="Escape"){if(!ui.panel.classList.contains("is-hidden"))closePanel(ui.panel);else if(!ui.dialogue.classList.contains("is-hidden"))advanceDialogue();return}if(event.key==="ArrowLeft"||event.key.toLowerCase()==="a")keys.left=true;if(event.key==="ArrowRight"||event.key.toLowerCase()==="d")keys.right=true;if((event.key==="Enter"||event.key===" ")&&!ui.dialogue.classList.contains("is-hidden"))return advanceDialogue();if(event.key.toLowerCase()==="e"||event.key==="Enter")interact()}
...
function showSolveStage(){ui.solveStage.classList.remove("is-hidden");ui.success.classList.add("is-hidden");ui.solveStep.className="is-active";ui.checkStep.className=""}
function showSuccess(){ui.solveStage.classList.add("is-hidden");ui.success.classList.remove("is-hidden");ui.solveStep.className="is-complete";ui.checkStep.className="is-complete"}
function submitAges(event){event.preventDefault();const form=new FormData(ui.answerForm),result=evaluateAges(form.get("younger"),form.get("older"));ui.answerFeedback.textContent=result.message;ui.answerFeedback.className=`form-feedback ${result.ok?"is-success":"is-error"}`;if(!result.ok)return;state.agesSolved=true;ui.objective.textContent="현재 나이의 차와 3년 뒤 나이의 합을 검산하자.";setTimeout(()=>{showSuccess();ui.dropChain.focus()},500)}
function showAnswerHint(){const hints=["동생의 현재 나이를 x살, 형의 현재 나이를 y살로 두어 봐.","현재 나이의 차는 y-x=4로 나타낼 수 있어.","3년 뒤 나이의 합은 (x+3)+(y+3)=40이야. 정리하면 x+y=34가 돼.","y-x=4와 x+y=34를 더하면 y를 구할 수 있어."];ui.answerFeedback.textContent=`힌트 · ${hints[Math.min(state.hintIndex,hints.length-1)]}`;ui.answerFeedback.className="form-feedback";state.hintIndex++}
function openPassage(){closePanel(ui.panel);state.passageOpen=true;ui.objective.textContent="형제가 비킨 골목을 지나 철교 너머로 이동하자.";showDialogue([{speaker:"WH",avatar:"WH",text:`동생은 ${AGE_DATA.youngerAge}살, 형은 ${AGE_DATA.olderAge}살이야. 지금 나이 차는 4살이고, 3년 뒤에는 18살과 22살이라 합이 40살이지.`},{speaker:"동생",avatar:"동생",text:"현재의 차와 3년 뒤의 합이 둘 다 맞네. 정답이야."},{speaker:"형",avatar:"형",text:"약속은 약속이지. 돈은 됐고 그냥 지나가. 쇠사슬 내릴게."},{speaker:"WH",avatar:"WH",text:"그리고 다시는 사람 길을 막고 돈을 요구하지 마."}],()=>showToast("쇠사슬이 내려갔어. 철교 너머 길이 열렸다!"))}
...
function showLine(){const line=state.dialogueQueue[0];ui.speaker.textContent=line.speaker;ui.avatar.textContent=line.avatar;ui.avatar.style.background=line.avatar==="WH"?"#1b8f88":line.avatar==="형"?"#70464d":"#5d526f";ui.dialogueText.textContent=line.text;ui.dialogueNext.focus()}
...
function openPanel(panel){clearMovement();panel.classList.remove("is-hidden");panel.querySelector("input,button")?.focus()}
